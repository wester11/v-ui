...
// EnsureSchemaUpgrades applies idempotent runtime upgrades for existing installs.
func EnsureSchemaUpgrades(ctx context.Context, db *pgxpool.Pool) error {
	statements := []string{
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS vpn_configs_active_per_server ON vpn_configs(server_id) WHERE is_active = TRUE`,
		`DO $$
		BEGIN
			IF EXISTS (
				SELECT 1 FROM pg_constraint WHERE conname='servers_protocol_check'
			) THEN
				ALTER TABLE servers DROP CONSTRAINT servers_protocol_check;
			END IF;
		END $$`,
		`ALTER TABLE servers ADD CONSTRAINT servers_protocol_check CHECK (protocol IN ('none','wireguard','amneziawg','xray'))`,
		`UPDATE servers SET status = CASE WHEN online THEN 'online' ELSE 'offline' END WHERE COALESCE(status, '') = ''`,
	}

	for _, stmt := range statements {
		if _, err := db.Exec(ctx, strings.TrimSpace(stmt)); err != nil {
			// constraint may already exist with same definition on some DB states
